#!/usr/bin/env python3
"""usage-board — self-hosted multi-provider AI usage dashboard."""
...
def _spend_path() -> Path:
    """Compact spend series next to snapshots.jsonl (same dir tests patch)."""
    return SNAPSHOT_PATH.with_name("spend.jsonl")
...
SPEND_SERIES_DAYS = 8
# Today + 30 complete prior calendar days (MSK display table).
...
app = FastAPI(title="usage-board", version="1.0.0")

# --- Optional HTTP Basic Auth (self-host deployments) ----------------------
# Set BOTH BASIC_AUTH_USER and BASIC_AUTH_PASSWORD to protect every route
# (UI and API). Set neither to run open (fine on localhost / private nets).
# Setting exactly one is a configuration error: the app refuses to start.
_BASIC_AUTH_USER = os.environ.get("BASIC_AUTH_USER", "").strip()
_BASIC_AUTH_PASSWORD = os.environ.get("BASIC_AUTH_PASSWORD", "")
_auth_secret = secrets.compare_digest


def _basic_auth_configured() -> bool:
    return bool(_BASIC_AUTH_USER) or bool(_BASIC_AUTH_PASSWORD)
...
def _basic_auth_valid(header_value: str) -> bool:
    ...
    if not sep:
        return False
    return _auth_secret(user, _BASIC_AUTH_USER) and _auth_secret(
...
if bool(_BASIC_AUTH_USER) != bool(_BASIC_AUTH_PASSWORD):
    raise RuntimeError(
        ...
        "(set both, or neither to disable auth)."
    )

if _basic_auth_configured():
    _WWW_AUTHENTICATE = 'Basic realm="usage-board", charset="UTF-8"'
...
def save_state(state: dict[str, Any]) -> None:
    ...
    with _spend_io_lock:
        _ensure_spend_backfill_unlocked()
        path = _spend_path()
        path.parent.mkdir(parents=True, exist_ok=True)
        with path.open("a", encoding="utf-8") as f:
            f.write(_spend_row_json(payload) + "\n")
    _rotate_snapshots_if_needed()


_snapshot_rows_cache: list[dict[str, Any]] | None = None
_snapshot_offset: int = 0
_last_snapshot_rotate: float = 0.0
_spend_io_lock = threading.Lock()

# Retention for spend.jsonl: rows older than this are trimmed (default 14d).
SNAPSHOT_RETENTION_HOURS = int(os.environ.get("SNAPSHOT_RETENTION_HOURS", "336"))
# Rotation is throttled: at most one trim pass per interval (default hourly).
SNAPSHOT_ROTATE_INTERVAL_SECONDS = int(os.environ.get("SNAPSHOT_ROTATE_INTERVAL_SECONDS", "3600"))


def invalidate_snapshot_cache() -> None:
    global _snapshot_rows_cache, _snapshot_offset
    _snapshot_rows_cache = None
    _snapshot_offset = 0


def _malloc_trim() -> None:
    """Return free glibc heap pages to the OS. No-op if libc/malloc_trim is missing."""
    try:
...
def _release_snapshot_heap() -> None:
    """Drop parse arenas after a one-shot fat snapshots.jsonl backfill.

    Incremental spend.jsonl ticks must not call this: gc.collect every poll
    would regress the CPU-peg fix (2026-08-26 / 2026-09-07). Compact spend
    rows do not need gc/trim on load or rotate.
    """
    gc.collect()
    _malloc_trim()


def _spend_row_json(obj: dict[str, Any]) -> str:
    return json.dumps(
        _compact_snapshot_row(obj),
        ensure_ascii=False,
        separators=(",", ":"),
    )


def _spend_history_available() -> bool:
    path = _spend_path()
    try:
...
def _ensure_spend_backfill() -> bool:
    with _spend_io_lock:
        return _ensure_spend_backfill_unlocked()


def _ensure_spend_backfill_unlocked() -> bool:
    """Fill spend.jsonl from snapshots.jsonl once. Snapshots are read-only."""
    dest = _spend_path()
    try:
    ...
    try:
        ...
        return False
    tmp = dest.with_name("spend.jsonl.backfill.tmp")
    wrote = False
    try:
        dest.parent.mkdir(parents=True, exist_ok=True)
        with SNAPSHOT_PATH.open("r", encoding="utf-8", errors="ignore") as src:
    ...
    if wrote:
        ...
        _release_snapshot_heap()
    return wrote


def _compact_snapshot_row(obj: dict[str, Any]) -> dict[str, Any]:
    """Spend-series projection: only fields spend_24h / spend_7d / series extractors read.

    Written to spend.jsonl (~500 B/row). Full wallets stay in historical
    snapshots.jsonl (read-only backfill source, not rewritten).
    """
    ...
    wallets_in = obj.get("wallets")
    wallets_out: dict[str, Any] = {}
    if isinstance(wallets_in, dict):
        ds = wallets_in.get("deepseek")
        if isinstance(ds, dict) and ds.get("balance") is not None:
            wallets_out["deepseek"] = {"balance": ds.get("balance")}

        orw = wallets_in.get("openrouter")
        if isinstance(orw, dict) and orw.get("total_usage") is not None:
            wallets_out["openrouter"] = {"total_usage": orw.get("total_usage")}

        zai = wallets_in.get("zai")
        if isinstance(zai, dict):
            weekly = zai.get("weekly")
            if isinstance(weekly, dict) and weekly.get("currentValue") is not None:
                wallets_out["zai"] = {"weekly": {"currentValue": weekly.get("currentValue")}}

        cc = wallets_in.get("commandcode")
        if isinstance(cc, dict):
            slim_cc: dict[str, Any] = {}
            monthly = cc.get("monthly")
            if isinstance(monthly, dict) and monthly.get("remaining_usd") is not None:
                slim_cc["monthly"] = {"remaining_usd": monthly.get("remaining_usd")}
            if cc.get("monthly_credits") is not None:
                slim_cc["monthly_credits"] = cc.get("monthly_credits")
            if slim_cc:
                wallets_out["commandcode"] = slim_cc

        kimi = wallets_in.get("kimi")
        if isinstance(kimi, dict):
            weekly = kimi.get("weekly")
            if isinstance(weekly, dict) and weekly.get("used") is not None:
                wallets_out["kimi"] = {"weekly": {"used": weekly.get("used")}}

        og = wallets_in.get("opencode-go")
        if isinstance(og, dict):
            monthly = og.get("monthly")
            if isinstance(monthly, dict) and monthly.get("used_usd") is not None:
                wallets_out["opencode-go"] = {"monthly": {"used_usd": monthly.get("used_usd")}}

    if wallets_out:
        compact["wallets"] = wallets_out

    accounts_raw = obj.get("accounts")
    if isinstance(accounts_raw, list):
        accounts_out: list[dict[str, Any]] = []
        for acc in accounts_raw:
            if not isinstance(acc, dict):
                continue
            if acc.get("provider") == "deepseek" or acc.get("email") == "deepseek-main":
                quota = acc.get("quota") or {}
                if isinstance(quota, dict) and quota.get("balance") is not None:
                    accounts_out.append({
                        "provider": acc.get("provider"),
                        "email": acc.get("email"),
                        "quota": {"balance": quota.get("balance")},
                    })
        if accounts_out:
            compact["accounts"] = accounts_out
    return compact
...
def load_snapshot_rows() -> list[dict[str, Any]]:
    """Incremental parse of spend.jsonl (compact ~500 B rows).

    First call may backfill from historical snapshots.jsonl (read-only, one
    line at a time). After that restarts only parse spend.jsonl — no fat
    file. Cost per poll is O(new_bytes). Compact rebuild does not gc/trim.
    """
    global _snapshot_rows_cache, _snapshot_offset
    _ensure_spend_backfill()
    path = _spend_path()
    if not path.exists():
        invalidate_snapshot_cache()
        return []
    try:
        ...
        return []
    full_rebuild = _snapshot_rows_cache is None or size < _snapshot_offset
    if full_rebuild:
        _snapshot_rows_cache = []
        _snapshot_offset = 0
    if size == _snapshot_offset:
        return _snapshot_rows_cache
    try:
        with path.open("r", encoding="utf-8", errors="ignore") as fh:
            fh.seek(_snapshot_offset)
            data = fh.read()
    except Exception:
        return _snapshot_rows_cache or []
    _snapshot_offset = size
    if data and not data.endswith("\n"):
        # Partial trailing line (append in flight): don't consume it yet.
        head, sep, tail = data.rpartition("\n")
        data = head + sep if sep else ""
        _snapshot_offset = size - len(tail.encode("utf-8", "ignore"))
    for line in data.splitlines():
        ...
        try:
            ...
            continue
        if isinstance(obj, dict):
            _snapshot_rows_cache.append(_compact_snapshot_row(obj))
    del data
    return _snapshot_rows_cache


def _snapshot_row_in_retention(obj: dict[str, Any], cutoff: float) -> bool:
    pt = parse_snapshot_ts(obj.get("ts"))
    return pt is not None and pt[0] >= cutoff


def _rotate_snapshots_if_needed() -> None:
    """Trim spend.jsonl rows older than SNAPSHOT_RETENTION_HOURS (atomic, throttled).

    Copies original compact JSONL text. Historical snapshots.jsonl is not
    rewritten. No gc/trim — spend.jsonl is small.
    """
    global _snapshot_rows_cache, _snapshot_offset, _last_snapshot_rotate
    now = time.time()
    if now - _last_snapshot_rotate < SNAPSHOT_ROTATE_INTERVAL_SECONDS:
        return
    _last_snapshot_rotate = now
    path = _spend_path()
    if not path.exists():
        return
    rows = load_snapshot_rows()
    cutoff = datetime.now(timezone.utc).timestamp() - SNAPSHOT_RETENTION_HOURS * 3600
    kept = [r for r in rows if _snapshot_row_in_retention(r, cutoff)]
    if len(kept) == len(rows):
        return
    tmp: Path | None = None
    rotated = False
    try:
        tmp = path.with_name("spend.jsonl.rotate.tmp")
            ...
            with tmp.open("w", encoding="utf-8") as dst:
                for line in src:
                    ...
                    try:
                        ...
                        continue
                    if isinstance(obj, dict) and _snapshot_row_in_retention(obj, cutoff):
                        dst.write(raw + "\n")
        tmp.replace(path)
        rotated = True
    finally:
    ...
    if not rotated:
        return
    _snapshot_rows_cache = kept
    try:
        _snapshot_offset = path.stat().st_size
    except OSError:
        _snapshot_offset = 0
...
def provider_has_api_key(name: str) -> bool:
    """True when the same getter a probe uses has a non-blank key."""
    getter = _PROVIDER_KEY_GETTERS.get(name)
    if getter is None:
        return False
    key = getter()
...
def get_visible_providers() -> list[str]:
    """Allowlist ∩ configured API key. Missing/blank key hides the card."""
    return [name for name in get_enabled_providers() if provider_has_api_key(name)]


def _wallet_probe_key(name: str) -> str:
    return f"{name}-main"


def _filter_visible_wallets(
    wallets: dict[str, Any] | None,
    visible: list[str] | None = None,
) -> dict[str, Any]:
    names = visible if visible is not None else get_visible_providers()
    src = wallets if isinstance(wallets, dict) else {}
    return {key: src[key] for key in names if key in src}
...
def hide_partial_spend_chips() -> bool:
    """Hide partial snapshot 24h/7d chips (often partial, shown with ~)."""
...
DASHBOARD_USER_AGENT = "usage-board/1.0 (+https://github.com/Rast53/usage-board)"
...
def compute_deepseek_spend_24h(
    ...
    if not _spend_history_available():
...
def compute_spend_series_7d(
    ...
    *,
    ...
    Default tz is UTC (8-day sparkline). spent[day] = delta(
...
def collect_state(force_quota: bool = False) -> dict[str, Any]:
    ...
    visible = get_visible_providers()
    visible_set = set(visible)
    required_keys = tuple(_wallet_probe_key(name) for name in visible)
    ...
    getter_names = tuple(_PROVIDER_KEY_GETTERS)
    if getter_names != KNOWN_PROVIDERS:
        raise RuntimeError(f"provider key getter mismatch: {getter_names}")
            ...
            for name, fn_name, err_label in _PROVIDER_PROBE_SPECS:
                ...
                try:
                    ...
                    if result is not None:
                        probed[_wallet_probe_key(name)] = result
                except Exception as e:
    ...
    for name in visible:
        ...
        wallet = builder(quota_accounts.get(_wallet_probe_key(name)))
        if wallet:
            wallets[name] = wallet

    notes = [text for name, text in _PROVIDER_NOTES if name in visible_set]
    notes.append(
...
def _startup() -> None:
    ...
    try:
        ...
        with _lock:
            _state["errors"] = list(_state.get("errors") or []) + [f"spend-backfill: {e}"]
...
def summary() -> dict[str, Any]:
    ...
    data["wallets"] = _filter_visible_wallets(wallets, visible)
    ...
    if isinstance(notes, list) and visible_set != set(KNOWN_PROVIDERS):
        keep = {text for name, text in _PROVIDER_NOTES if name in visible_set}
        data["notes"] = [
            note
            for note in notes
...
def wallets() -> dict[str, Any]:
    with _lock:
        raw = _state.get("wallets") or {}
        return {
            "updated_at": _state.get("updated_at"),
            "wallets": _filter_visible_wallets(raw if isinstance(raw, dict) else {}),
        }
...
def pace() -> dict[str, Any]:
    ...
    payload = build_pace_payload(cache)
    keep = {_wallet_probe_key(name) for name in get_visible_providers()}
    payload["accounts"] = [
        acc for acc in (payload.get("accounts") or [])
        if str(acc.get("provider") or "") in keep
    ]
    payload["no_window"] = [
...
def index() -> HTMLResponse:
    ...
    text = text.replace(
        "<title>Мои подписки · usage-board</title>",
        f"<title>{title} · usage-board</title>",
        1,
    )
